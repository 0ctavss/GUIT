...
void resetFile(const std::string& filename) {  // Aquí filename es el nombre del archivo que se quiere resetear
...
void processCommand(const std::vector<std::string>& args) {
    ...
    if (command == "init" && args.size() == 3) {
        ...
        std::string base_path = "C:/TEC/Commits";
        ...
        if (args.size() == 3) {
            ...
            std::cout << "Usage: guit reset <file>" << std::endl;
        }
        ...
        if (args.size() >= 3) {
            ...
            std::cout << "Usage: guit status <file>" << std::endl;
        }
        ...
        // Aquí iría la lógica para realizar el commit
    } else if (command == "sync" && args.size() == 3) {
        std::string filename = args[2];
        std::cout << "Synchronizing file: " << filename << std::endl;
        // Aquí iría la lógica para sincronizar el archivo
    } else if (command == "quit") {
        std::cout << "Exiting the program." << std::endl;
        exit(0);
    } else {
        std::cout << "Invalid command or usage. Type 'guit help' for a list of commands." << std::endl;
    }
}
